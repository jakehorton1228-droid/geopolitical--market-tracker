"""Drop Silver tables — hand ownership to dbt

Revision ID: a1f2b3c4d5e6
Revises: d4e5f6a7b8c9
Create Date: 2026-07-17

The Silver layer is now built by dbt (dbt-duckdb models in backend/dbt_project/
models/silver/). dbt materializes these tables itself with CREATE TABLE, so
Alembic must NOT pre-create them: dbt's DROP+CREATE cannot drop an Alembic table
that carries indexes (DuckDB's postgres extension errors on it). This migration
drops the Alembic-owned Silver tables so dbt has clean ownership.

Bronze (raw ingestion) stays owned by Alembic. Gold was already dbt-owned.

Reversible: downgrade recreates the tables as they were defined in
d4e5f6a7b8c9, for anyone rolling back to the pre-dbt Python transforms.
"""
...
SILVER_TABLES = [
    "silver_event_market",  # depends on the others — drop first
    "silver_headlines",
    "silver_market",
    "silver_events",
]
...
def upgrade() -> None:
    # CASCADE + IF EXISTS: works whether the table is the original Alembic one
    # (with indexes) or a dbt-created one from an interim run.
    for table in SILVER_TABLES:
        op.execute(f"DROP TABLE IF EXISTS {table} CASCADE")


def downgrade() -> None:
    # Recreate the Silver tables exactly as d4e5f6a7b8c9 defined them, so a
    # rollback restores the pre-dbt (Python transform) schema.
    op.create_table(
        'silver_events',
